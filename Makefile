...
add_person_cpp: add_person.cc MessagePublisher.cpp protoc_middleman
	pkg-config --cflags protobuf nanomsg # fails if protobuf is not installed
	c++ -std=c++11 -std=gnu++11 add_person.cc MessagePublisher.cpp fatal.cpp addressbook.pb.cc -o  add_person_cpp `pkg-config --cflags --libs protobuf nanomsg`

list_people_cpp: list_people.cc MessageSubscriber.cpp MessageSubscriber.h protoc_middleman
	pkg-config --cflags protobuf nanomsg # fails if protobuf is not installed
	c++ -std=c++11 -std=gnu++11 list_people.cc MessageSubscriber.cpp MessageSubscriber.h fatal.cpp addressbook.pb.cc -o list_people_cpp `pkg-config --cflags --libs protobuf nanomsg`
